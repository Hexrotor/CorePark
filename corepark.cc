...
void dynamicCoreControl(struct CPU** cpuList, size_t coreCount) {
    ...
    float minCPUFreqPercentage = 1;
    for(int i = 0; i < coreCount; i++) {
        cpuList[i]->freqPercentage = (float)(cpuList[i]->freq - cpuList[i]->minFreq) / (cpuList[i]->maxFreq - cpuList[i]->minFreq);
        if(cpuList[i]->online && cpuList[i]->freqPercentage < minCPUFreqPercentage) {
            minCPUFreqPercentage = cpuList[i]->freqPercentage;
        }
    ...
    for(int i = E_CORE_START - 1; i >= 1; i--) {
        if(cpuList[i]->online == 1 && (cpuList[i]->usage < 0.01 || cpuList[i]->freqPercentage < 0.1)) {
    ...
    if(minCPUFreqPercentage > 0.1) {
        for(int i = E_CORE_START + 1; i < coreCount; i++) {
            if(cpuList[i]->online == 0) {
    ...
    if(AvgUsage > 0.3 && minCPUFreqPercentage > 0.1) {
